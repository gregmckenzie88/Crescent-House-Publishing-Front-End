...
import { getSampleChapterContent } from "@/app/actions";
...
export function SampleChapter({ 
  ...
  useEffect(() => {
    if (isOpen && contentRef.current && !isLoading) {
  ...
  const handleOpen = async () => {
    ...
    if (!content && slug) {
      setIsLoading(true);
      try {
        const data = await getSampleChapterContent(slug);
        setFetchedContent(data);
      } catch (error) {
        console.error("Failed to fetch sample chapter:", error);
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const triggerButtonClass = triggerClassName || "h-14 px-8 border-primary/40 text-primary hover:bg-transparent hover:text-primary transition-all uppercase tracking-widest text-sm font-medium";
            ...
            >
              {isLoading ? (
                <div className="flex flex-col items-center justify-center py-20">
                  ...
                  <p className="text-muted-foreground animate-pulse">Loading first chapter...</p>
                </div>
              ) : (
